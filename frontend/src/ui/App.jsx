...
export default function App() {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3001/render', {
        ...
        body: JSON.stringify({
          ...
          words: result.words,
          videoUrl: serverVideoUrl || videoSrc,
        }),
      });
  ...
  }, [selectedPreset, result.words, serverVideoUrl, videoSrc]);

  const downloadPropsJson = useCallback(() => {
    try {
      const json = JSON.stringify({ ...baseProps, preset: selectedPreset }, null, 2);
      ...
      <div className="hero">
        <div className="hero-card">
          ...
          <div className="form-group">
            <input type="file" accept="video/*" onChange={onFileChange} className="input-file" />
            <button
          ...
          {result.text && <div className="success-msg">Captions generated!</div>}
          ...
          {result.words.length > 0 && (
            <div className="player-preview">
              <Player
                ...
                inputProps={{ words: result.words, preset: selectedPreset, videoSrc }}
              />
            </div>
          )}
